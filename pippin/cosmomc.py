...
class CosmoMC(Task):  # TODO: Define the location of the output so we can run the lc fitting on it.
    ...
    def __init__(self, name, output_dir, options, dependencies=None):
        ...
        self.chain_dict = {l: os.path.join(self.chain_dir, "chain" + i.replace(".ini", f"_{n + 1}.txt")) for l, i in zip(self.covopts, self.ini_files) for n in range(self.num_walkers)}
