    ...
    def get_tasks(c, prior_tasks, base_output_dir, stage_number, prefix, global_config):
        ...
        for clas_name in c.get("CLASSIFICATION", []):
            ...
            if mode == Classifier.PREDICT and options.get("SIM_FRACTION", 1) > 1:
                Task.fail_config(
                    "SIM_FRACTION must be 1 (all sims included) for predict mode"
                )
                ...
                if opt_sim:
                    if not any([mask, mask_sim]):
                        ...
                        if len(optional_sim_tasks) == 0:
                            Task.logger.warn(
                                f"Optional SIM dependency but no matching sim tasks for MASK: {mask} or MASK_SIM: {mask_sim}"
                            )
                        else:
                            Task.logger.debug(
                                f"Found {len(optional_sim_tasks)} optional SIM dependencies"
                            )
                    ...
                    if not any([mask, mask_fit]):
                        ...
                        for l in lcfit_tasks:
                            ...
                            if len(optional_lcfit_tasks) == 0:
                                Task.logger.warn(
                                    f"Optional LCFIT dependency but no matching lcfit tasks for MASK: {mask} or MASK_FIT: {mask_fit}"
                                )
                            else:
                                Task.logger.debug(
                                    f"Found {len(optional_lcfit_tasks)} optional LCFIT dependencies"
                                )
            ...
            for run in runs:
                ...
                if mask_combined:
                    ...
                    if len(run) > 1:
                        Task.logger.warn(
                            f"Classifier {name} has multiple tasks -- this should only occur when COMBINE_MASK is specified. Using first task."
                        )
                ...
                sim_deps = [
                    sim_fit_tuple[0]
                    for sim_fit_tuple in run
                    if sim_fit_tuple[0] is not None
                ]
                fit_deps = [
                    sim_fit_tuple[1]
                    for sim_fit_tuple in run
                    if sim_fit_tuple[1] is not None
                ]
                ...
                sim_name = (
                    "_".join([s.name for s in sim_deps if s is not None])
                    if len(sim_deps) > 0
                    else None
                )
                fit_name = (
                    "_".join([l.name for l in fit_deps if l is not None])
                    if len(fit_deps) > 0
                    else None
                )
                ...
                if model is not None:
                    if "/" in model or "." in model:
                        ...
                        if len(tasks) == 0:
                            Task.fail_config(
                                f"Your model {model} has not yet been defined."
                            )
                        for t in tasks:
                            if model == t.name:
                                # deps.append(t)
